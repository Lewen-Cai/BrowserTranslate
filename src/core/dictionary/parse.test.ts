...
describe('parseDictionaryEntry', () => {
  it('parses a complete entry', () => {
    ...
    expect(e!.phonetic).toBe('/ˌsɛrənˈdɪpɪti/');
  ...
  it('treats an empty phonetic as absent (no pinyin for CJK)', () => {
    const e = parseDictionaryEntry(JSON.stringify({ headword: '模块', phonetic: '', partOfSpeech: '名词', senses: ['标准化的可组合单元。'], example: null }));
    expect(e).not.toBeNull();
