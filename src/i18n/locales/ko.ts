import type { StringKey } from './en';

export const ko: Record<StringKey, string> = {
  // Common
  loading: '불러오는 중…',
  save: '저장',
  cancel: '취소',
  delete: '삭제',
  edit: '편집',
  close: '닫기',
  clone: '복제',
  clearAll: '전체 지우기',
  // Popup / options shared
  settings: '설정',
  openFullSettings: '전체 설정 열기',
  privacyTagline: '오픈소스 · 내 키 사용 · 중계 없음 · 텔레메트리 없음',
  ready: '준비됨',
  notConfigured: '설정되지 않음',
  // Sections
  sectionApi: 'API',
  sectionTranslation: '번역',
  sectionRecent: '최근 번역',
  sectionApiEndpoint: 'API 엔드포인트',
  sectionCacheHistory: '캐시 및 기록',
  sectionAppearance: '모양',
  sectionData: '데이터',
  sectionPromptTemplates: '프롬프트 템플릿',
  sectionHistory: '번역 기록',
  openaiCompatible: 'OpenAI 호환',
  // Options nav
  navApi: 'API',
  navPrompts: '프롬프트',
  navGeneral: '일반',
  navHistory: '기록',
  // Provider type + presets
  providerType: '제공자 유형',
  providerTypeCloud: '클라우드',
  providerTypeLocal: '로컬',
  cloudProvider: '제공자',
  cloudProviderCustom: '사용자 지정',
  cloudEndpoint: '엔드포인트',
  applyConfig: '설정 적용',
  // Status states
  statusChecking: '확인 중…',
  statusModelMissing: '모델을 찾을 수 없음',
  statusOffline: '오프라인',
  // API form
  baseUrl: 'Base URL',
  apiKey: 'API Key',
  model: '모델',
  defaultPromptTemplate: '기본 프롬프트 템플릿',
  prompt: '프롬프트',
  // Translation form
  targetLanguage: '대상 언어',
  triggerMode: '실행 방식',
  hotkey: '단축키',
  hotkeyHint: '모든 페이지에서 두 가지 방식 모두 작동합니다.',
  iconAfterSelection: '선택 후 아이콘 표시 (기본값)',
  hotkeyOnly: '단축키만 — 아이콘 없음',
  // Full-page translation
  translatePage: '페이지 번역',
  showOriginal: '원문 표시',
  translateFailed: '번역 실패',
  retry: '다시 시도',
  noTranslationNeeded: '이미 대상 언어입니다',
  fullPageHotkey: '전체 페이지 단축키',
  // Behavior
  cacheTranslations: '번역 캐시',
  cacheDesc: '동일한 텍스트의 번역을 재사용하여 토큰 절약',
  cacheTtl: '캐시 유지 기간(일)',
  saveHistory: '번역 기록 저장',
  saveHistoryDesc: '로컬에만 저장되며 어디에도 전송되지 않습니다',
  historyMax: '기록 최대 개수',
  cacheShort: '캐시',
  historyShort: '기록',
  // Appearance
  theme: '테마',
  themeAuto: '시스템 설정 따름',
  themeLight: '라이트',
  themeDark: '다크',
  uiLanguage: '인터페이스 언어',
  uiLangAuto: '시스템 설정 따름',
  // Prompt templates
  newTemplate: '새 템플릿',
  builtin: '기본 제공',
  systemPrompt: '시스템 프롬프트',
  userPromptTemplate: '사용자 프롬프트 템플릿',
  userPromptTemplateHint: '변수 · {{text}}(필수) · {{targetLang}} · {{sourceLang}} · {{url}} · {{title}}',
  viewReadonly: '보기 · 읽기 전용',
  editTemplate: '템플릿 편집',
  // History
  searchHistory: '원문 또는 번역 검색...',
  noEntries: '기록 없음',
  noEntriesHint: '번역하면 여기에 표시됩니다.',
  exportSettings: '설정 내보내기',
  importSettings: '설정 가져오기',
  dataSectionDesc: '설정을 JSON 파일로 백업하거나 이전할 수 있습니다. 기록과 캐시는 포함되지 않습니다.',
  includeApiKeys: 'API 키 포함',
  includeApiKeysWarning: '내보낸 파일에 API 키가 평문으로 포함됩니다. 안전하게 보관하세요.',
  importSuccess: '설정을 가져왔습니다.',
  importFailed: '가져오기 실패',
  // Translation card (Shadow DOM)
  cardExample: '예문',
  cardRefreshNeeded: '확장 프로그램이 업데이트되었습니다. 계속하려면 이 페이지를 새로고침하세요.',
  // Errors
  noProfileError: 'API가 설정되지 않았습니다. 설정을 열어 키를 추가하세요.',
};
